...
namespace TinyRenderer
{
	...
	int TRRenderer::addSpotLight(glm::vec3 pos, glm::vec3 atten, glm::vec3 dir, glm::vec3 color, float cutOff , float outerCutOff)
	{
		return TRShadingPipeline::addSpotLight(pos, atten, dir , color , cutOff , outerCutOff);
	}
	...
	void TRRenderer::clearSpotLight()
	{
		TRShadingPipeline::clearSpotLight();
	}
		...
		for (size_t m = 0; m < m_drawableMeshes.size(); ++m)
		{
			...
			for (size_t f = 0; f < faces.size(); ++f)
			{
				//Setup the shading options
				{
					...
					// printf("%f ", m_shader_handler->m_shininess);
