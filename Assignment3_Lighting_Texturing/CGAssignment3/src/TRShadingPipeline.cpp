...
namespace TinyRenderer
{
	...
	int TRShadingPipeline::addSpotLight(glm::vec3 pos, glm::vec3 atten, glm::vec3 dir , glm::vec3 color , float cutOff , float outerCutOff)
	{
		m_spot_lights.push_back(TRSpotLight(pos, atten, dir ,color , cutOff , outerCutOff));
	...
	void TRPhongShadingPipeline::fragmentShader(const VertexData &data, glm::vec4 &fragColor)
	{
		...
		for (size_t i = 0; i < m_point_lights.size(); ++i)
		{
			...
			// 环境光计算
			ambient = glm::vec3(light.lightColor.x * amb_color.x , light.lightColor.y * amb_color.y , light.lightColor.z * amb_color.z);

			// 漫反射光计算
			float cos_theta = glm::max(0.0f , glm::dot(normal, lightDir));
			diffuse = glm::vec3(light.lightColor.x * dif_color.x, light.lightColor.y * dif_color.y, light.lightColor.z * dif_color.z) * cos_theta;
			
			// diffuse = glm::vec3(m_kd.x * dif_color.x, m_kd.y * dif_color.y, m_kd.z * dif_color.z) * cos_theta;			
			// 镜面反射光计算
			// glm::vec3 r = glm::reflect(normal , lightDir);
			// float cof = glm::pow(glm::dot(r, viewDir) , m_shininess);
			// specular = glm::vec3(light.lightColor.x * spe_color.x, light.lightColor.y * spe_color.y, light.lightColor.z * spe_color.z) * cof;
			// bling-phong
			glm::vec3 halfwayDir = glm::normalize(lightDir + viewDir);
			float cof = glm::pow(glm::max(glm::dot(normal, halfwayDir), 0.0f), m_shininess);
			specular = glm::vec3(light.lightColor.x * spe_color.x, light.lightColor.y * spe_color.y, light.lightColor.z * spe_color.z) * cof;
			/*ambient = amb_color;*/
			// diffuse = dif_color;
			// specular = spe_color;
			...
			//{
			fragColor.x += (ambient.x + diffuse.x + specular.x) * attenuation; // * intensity;
			fragColor.y += (ambient.y + diffuse.y + specular.y) * attenuation; // * intensity;
			fragColor.z += (ambient.z + diffuse.z + specular.z) * attenuation; // * intensity;
			//}
